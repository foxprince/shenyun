...
(function($){
	$.fn.Huploadify = function(opts){
		...
		var defaults = {
			...
			showUploadedQueue: true, //是否显示上传的进度队列
			buttonText: '选择文件',//上传按钮上的文字
			buttonCss: '',  //上传按钮的样式
			uploadButton: '.uploadbtn', //触发上传时间的按钮对象
		...
		this.each(function(index, element){
			...
			var uploadManager = {
				...
				init : function(){
					...
					inputStr += '"/>';
					inputStr += '<a id="file_upload_' + instanceNumber + '-button" href="javascript:void(0)" class="uploadify-button ' + option.buttonCss + '">';
					inputStr += option.buttonText;
					inputStr += '</a>';
					var uploadFileListStr = '<div id="file_upload_' + instanceNumber + '-queue" class="uploadify-queue" ' + (!option.showUploadedQueue ? 'style="display:none;"' : '') + '></div>';
					...
					returnObj =  {
						...
						fileCount:function(){
						    return uploadManager.filteredFiles.length;
						},
						...
						settings : function(name,value){
							if(arguments.length==1){
							    ...
							    if (name == 'formData') {
							        //动态传参 upload_file_nv.settings('formData', { 'aa': 'bb' })
				...
				_filter: function (files, e) {		//选择文件组的过滤方法
					var arr = [];
						...
						for(var i=0,len=files.length;i<len;i++){
							...
							if(parseInt(F.formatFileSize(f.size,true))<=option.fileSizeLimit){
								if($.inArray('*',typeArray)>=0 || $.inArray(f.name.split('.').pop(),typeArray)>=0){
								    ...
								    alert('只允许' + option.fileTypeExts + '类型的文件！');
								}
							}
							else {
				...
				_renderFile : function(file){
				    ...
				    //如果是非自动上传，显示上传按钮
					if (!option.auto && $html.find(option.uploadButton).length > 0) {
					    $html.find(option.uploadButton).css('display', 'inline-block');
					}
					...
					if(option.auto){
					    ...
					    //如果配置非自动上传，绑定上传事件
					    var upload_button = $html.find(option.uploadButton);
					    if (upload_button.length == 0)
					        upload_button = $(option.uploadButton);
				...
				_getFiles: function (e) {
			  		...
			  		files = uploadManager._filter(files,e);
